...
use h3_raster::tile::tile_size_from_rasterband;
...
fn convert_r_dataset(h3_res: u8, n_threads: usize) {
    ...
    let band_idx = 1;
    let band = ds.rasterband(band_idx).unwrap();
    let tile_size = tile_size_from_rasterband(&band, n_threads);
